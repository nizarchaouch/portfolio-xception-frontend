...
<template>
  ...
  >
    ...
    <v-row>
      <v-col cols="12" :class="'mt-9 text-' + settings.titre.justify">
        <v-color-picker
          v-if="showColorCarre && !voir"
          @click="saveForm()"
          v-model="this.settings.carre.color"
          :modes="['hexa']"
          class="mx-auto mb-2"
        ...
        <v-icon
          class="mb-6 me-2"
          size="x-large"
          :color="settings.carre.color"
          @click="showColorCarre = !showColorCarre"
          >mdi-square</v-icon
        >
        <div
          v-html="settings.titre.nom"
          class="text-h3 d-inline overflow-hidden"
          :class="{
